...
11. PPTX 先按真实幻灯片顺序提取原生文字；包含图片的页才调用本地 OCR Provider。OCR 不联网、不调用托管服务；引擎缺失、执行失败或置信度低且未提供校对正文时停止。
12. 完整页证据来源生成 03 知识卡时，每条事实必须绑定同页 `verbatim_text` 和 `evidence_sha256`；字符串概括不能绕过证据合同。
