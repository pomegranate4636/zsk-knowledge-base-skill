...
def root_content(binding: Binding, key: str) -> str:
    if key == "AGENTS":
        return (
            ...
            "资料入库只走 zsk-router；原件先进入 01，再按内容单元自动识别 03/04/05；分类不确定留在 01，不冒充异常写入 02。"
            "未经单独授权，不生产、发布、发送或改变权限。"
    ...
    if key == "README":
        return (
            ...
            "隐私、权限、归属、版本、主体或转换回读异常会停在 02；普通分类不确定仍留在 01。AGENTS、README 和 06 可由客户修改。"
    ...
    if key == "06":
        return (
            ...
            "## 入库偏好（客户可修改）\n\n"
            "- 分类方式：自动分类（也可改为“写入前预览”）\n"
            "- 页面视觉：按需询问（也可改为“仅文字”或“完整页证据”）\n"
            "- Profile：候选先入候选区，确认后再更新当前事实\n\n"
