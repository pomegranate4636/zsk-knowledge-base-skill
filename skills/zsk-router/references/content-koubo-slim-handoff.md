...
- output：`07-生产与反馈`
- Manifest：`06-Agent与Workflow/content-koubo-client-manifest.json`

## 执行

1. 从当前已回读的 ZSK Binding 取得 `client_id` 和 Obsidian 绝对路径，不重新生成或猜测。
2. 定位已安装的 `shared/configure_content_koubo_slim.py`。路径缺失或脚本不可读时停止。
...
6. 只有 Manifest、Registry、Runs 全部写入或复用并回读一致，才能说连接完成。Content 口播 Slim 尚未真实安装时，只能说配置已准备，不能说内容工作流可用。
7. 不读取、迁移、覆盖或删除旧 `.content-v2-slim`、旧 `content-client-manifest.json` 和旧 Run；它们只用于回滚和读取历史任务。

示意命令：

```bash
python3 /真实/Skills/shared/configure_content_koubo_slim.py \
...
Registry 已有其他客户时，可以在确认后原子合并当前客户并保留原记录。Content 口播 Slim 第一次运行只会在 Registry 恰好有一个客户时自动采用；多个客户必须由用户明确选择。
