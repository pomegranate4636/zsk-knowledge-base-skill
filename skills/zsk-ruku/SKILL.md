...
- `source_id` 是内容身份；01 的人类可见来源页标题必须为 `来源-SRC-YYYYMMDD-NNN`。其中日期为实际登记日期，`NNN` 为当日递增序号。原件文件名、可读版 H1、幻灯片标题和项目名称只能出现在正文或来源元数据中，绝不能覆盖该来源页标题。
- 房产 PPT 的 01 来源页必须同时保留可读 Markdown 和完整逐页幻灯片图片。图片清单必须记录页码、页图指纹和显示说明，并在发布后按页码回读核对；任一页缺失、重复、页数不一致或回读失败即停止，不得报告登记成功。
- 对只支持在文档末尾插图的后端，允许将全部逐页图片集中追加在来源正文末尾；但每张图仍必须具有明确页码说明，且不可用 PPT 内嵌素材图代替整页幻灯片。
