...
`source_id` 使用原件 SHA256，只写入可读版元数据；当前绑定中客户主动提交的资料默认具有处理权和原件保留授权，完成隐私检查后在 01 create-only 分开保存原件和带来源记录的可读 Markdown。客户可见名称使用“日期＋资料标题”，不使用 `SRC-...` 或哈希串做文件名。可读版必须记录转换器名称和版本。异常只在真实转换、写入或回读故障时写入 02。只返回 `registered / reused / exception`，不要求客户判断 03/04/05。

## 通用完整页证据

PDF/PPTX 默认不生成页图。只有 Router 明确传入 `page_evidence_mode=required` 时才启用；当前绑定中客户主动提交的资料默认允许永久保留原件。页图不代替 MarkItDown 文本，两者共同进入 01：
...
PDF/PPTX 均先按页提取原生文字：PDF 使用有效内嵌文字层，PPTX 使用可编辑文字。空白、模板占位或无法提取的页才交给本地 OCR Provider。图片页以 300 DPI 渲染，并由三次本地 OCR 自动验证：优先要求两次近乎一致；版式差异导致文字不能逐字一致时，只接受“主结果置信度至少 0.85 且另一结果可部分印证”的组合。客户不参与校对。任一页无法自动可靠还原时，整份资料返回 `file_quality_insufficient` 且零后端写入。OCR 不联网、不调用托管服务，也不做图片描述、图片检索、行业分类或猜测式图文对应。
...
当前绑定中客户主动提交的敏感原件默认允许私有保存；可读版仍须自动脱敏。未提交文件不得读取、保存或引用。
