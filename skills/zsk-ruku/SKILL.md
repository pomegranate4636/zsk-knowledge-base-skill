...
PPTX 按真实幻灯片顺序提取原生文字；包含图片的页才交给本地 OCR Provider。图片页以 300 DPI 渲染，并由三次本地 OCR 的一致性自动验证；客户不参与校对。任一页无法自动可靠还原时，整份资料返回 `file_quality_insufficient` 且零后端写入。OCR 不联网、不调用托管服务，也不做图片描述、图片检索、行业分类或猜测式图文对应。
